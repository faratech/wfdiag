...
| `wfdiag-native-issues` | issue catalog (34 `IssueSpec`s), pure detectors, UI projection, fix-plan validation, worker runtime | yes |
| `wfdiag-native-remediation` | remediation engine + `broker::ActionBroker` — the **only** execution path | yes |
| `wfdiag-native-diagnostics` | task catalog (48 tasks), Windows collectors, scan orchestration runtime | yes |
