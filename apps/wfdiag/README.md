...
official crates.io 0.100.0 release (adopted 2026-09-03, superseding the git-revision pin of
the same reviewed source). A branch, tag, floating git dependency, or any other version is
prohibited. Moving the pin means updating the three Cargo dependencies,
`reactor-baselines/manifest.json` (`reactor_pin`), `scripts/build-reactor-msix-probe.py`, and
the readiness script's `EXPECTED_REACTOR_*` constants together in one reviewed change.
`scripts/check-external-gates.py` watches crates.io for anything newer than the adopted
release.
...
- **Issues & remediation** — maintenance and per-issue Run buttons execute through the shared
  ...
  same broker and never executes directly. With the default-off setting "Let the assistant run safe fixes", the facade's automation layer approves a staged `AutoSafe` action with the plain review approval (the broker still refuses every Repair) and verifies it afterwards; see `CLAUDE.md` → Security model → "Automation never widens the gate". Subscription CLIs (Codex CLI, Claude Code): at startup the shell asks the facade for both accounts (a credential-cache look; the vendor status command only when a cache exists), the AI page shows a sign-in banner whenever an installed CLI is the only thing between the user and a working assistant, the Settings account row names the obstacle (no stored login, signed out, npm shim only, status unclear), a completed install hands off to Sign in without starting it, and Sign in opens the vendor's own console window. Ask AI immediately sends current issue evidence through
