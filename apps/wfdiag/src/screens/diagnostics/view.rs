...
use wfdiag_native_projection::task_output::{
    MAX_STRUCTURED_OUTPUT_BYTES, MAX_STRUCTURED_OUTPUT_ROWS, format_output_key_values,
};

/// The raw document view for one result. The parse/truncation policy lives
/// in `wfdiag-native-projection` (rule 2); this wrapper only maps the
/// shell's task types onto it (2026-09-03 audit).
pub(crate) fn diagnostic_raw_document(
    result: &DiagnosticTaskResult,
    task: Option<&DiagnosticTask>,
) -> String {
    wfdiag_native_projection::task_output::diagnostic_raw_document(
        &wfdiag_native_projection::task_output::RawDocumentInput {
            task_id: &result.task_id,
            task_name: task.map_or(result.task_id.as_str(), |task| task.name.as_str()),
...
mod tests {
    ...
    use wfdiag_native_projection::task_output::RAW_OUTPUT_TRUNCATION_NOTICE;
