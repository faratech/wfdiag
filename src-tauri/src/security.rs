...
impl SecureCommandExecutor {
    pub fn new() -> Self {
        ...
        allowed_commands.insert("powershell".to_string(), CommandConfig {
        ...
        allowed_commands.insert("verifier".to_string(), CommandConfig {
            executable: "verifier.exe".to_string(),
            allowed_args: vec!["/querysettings".to_string()],
            max_args: 1,
        });

        allowed_commands.insert("powercfg".to_string(), CommandConfig {
        ...
        allowed_commands.insert("dsregcmd".to_string(), CommandConfig {
            executable: "dsregcmd.exe".to_string(),
            allowed_args: vec!["/status".to_string()],
            max_args: 1,
        });

        allowed_commands.insert("wmic".to_string(), CommandConfig {
        ...
        allowed_commands.insert("wevtutil".to_string(), CommandConfig {
        ...
        allowed_commands.insert("netstat".to_string(), CommandConfig {
            executable: "netstat.exe".to_string(),
            allowed_args: vec!["-an".to_string()],
            max_args: 1,
        });

        allowed_commands.insert("ipconfig".to_string(), CommandConfig {
    ...
    pub fn execute_command(&self, cmd: &str, args: &[&str]) -> Result<std::process::Output> {
        ...
        let config = self.allowed_commands.get(cmd)
    ...
    pub fn execute_powershell_script(&self, script: &str) -> Result<std::process::Output> {
        ...
        let config = self.allowed_commands.get("powershell")
    ...
    fn validate_arguments(&self, cmd: &str, args: &[&str], config: &CommandConfig) -> Result<()> {
        match cmd {
            ...
            "wevtutil" => {
                ...
                if args.len() == 4 && args[0] == "qe" && 
                   (args[1] == "System" || args[1] == "Application") &&
                   args[2] == "/c:100" && args[3] == "/f:text" {
                    Ok(())
                } else {
            ...
            _ => {
                ...
                for arg in args {
                    if !config.allowed_args.contains(&arg.to_string()) && !self.is_safe_argument(arg) {
                        return Err(anyhow::anyhow!("Argument '{}' not allowed for '{}'", arg, cmd));
                    }
                }
                Ok(())
            }
    ...
    fn validate_powershell_script(&self, script: &str) -> Result<()> {
        ...
        let blocked_patterns = [
            ...
            "Start-Process", "New-Object", "Add-Type", "Invoke-RestMethod",
            "Invoke-WebRequest", "Download", "Remove-", "Delete", 
            "Set-", "New-", "Install-", "Uninstall-", "Stop-", "Start-Service",
            "&", ";", "$env:", "Get-Credential", "ConvertTo-SecureString",
        ...
        for pattern in &blocked_patterns {
            if script_lower.contains(&pattern.to_lowercase()) {
                return Err(anyhow::anyhow!("PowerShell script contains blocked pattern: {}", pattern));
            }
        }

        // Check if script uses allowed cmdlets (more permissive check)
        let has_allowed_cmdlet = allowed_cmdlets.iter().any(|cmdlet| {
            script_lower.contains(&cmdlet.to_lowercase())
        });
        ...
        if !has_allowed_cmdlet {
            ...
            let has_basic_cmdlet = basic_allowed.iter().any(|cmdlet| {
                script_lower.contains(&cmdlet.to_lowercase())
            });
            
            if !has_basic_cmdlet {
                return Err(anyhow::anyhow!("PowerShell script must use allowed diagnostic cmdlets"));
            }
        }
    ...
    fn validate_wmi_query(&self, args: &[&str]) -> Result<()> {
        ...
        if args.len() >= 2 && args[0] == "path" {
            ...
            if !allowed_wmi_classes.contains(&wmi_class) {
                return Err(anyhow::anyhow!("WMI class '{}' not in whitelist", wmi_class));
            }
        }
...
mod tests {
    ...
    fn test_powershell_validation() {
        ...
        assert!(executor.validate_powershell_script("Get-AppxPackage | Select-Object Name").is_ok());
        
        // Dangerous script should fail
        assert!(executor.validate_powershell_script("Invoke-Expression 'calc'").is_err());
        assert!(executor.validate_powershell_script("Start-Process notepad").is_err());
    }
}