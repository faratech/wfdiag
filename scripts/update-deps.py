...
def parse_cargo_toml(content: str) -> dict[str, tuple[str, int, int]]:
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^\[dependencies\]', line):
            ...
            in_build_deps = False
            continue
        elif re.match(r'^\[build-dependencies\]', line):
            in_deps = False
            in_target_deps = False
            in_build_deps = True
            continue
        ...
        if not (in_deps or in_target_deps or in_build_deps):
            continue
...
def main():
    ...
    print(f"Updating dependencies in: {cargo_toml}")
    ...
    print(f"{'Crate':<30} {'Current':<15} {'Latest':<15} {'Status'}")
    print("-" * 80)
    ...
    for crate_name, (current_version, _, _) in sorted(deps.items()):
        ...
        print(f"{crate_name:<30} {current_version:<15} {latest or 'N/A':<15} {status}")
