//! Every fallible entry point here returns the shipping `String` error the
//! two shells surface verbatim, so per-function `# Errors` sections would only
//! restate the message.
...
use std::sync::Arc;
use wfdiag_native_core::error::DiagError;
use wfdiag_native_core::timestamp::Timestamp;
...
impl ScanStorage {
        ...
        C: Fn() -> Vec<crate::diagnostics::DiagnosticTask> + Send + Sync + 'static,
    {
        ...
        println!("Scan storage initialized at: {}", storage_dir.display());
    ...
    pub fn load_scan(&self, scan_id: &str) -> Result<ScanRecord, String> {
        if !self.encrypted_storage.exists(scan_id) {
            return Err(DiagError::storage(
                "load_scan",
                format!("Encrypted scan '{scan_id}' not found"),
            )
            .into());
        }
    ...
    pub fn compare_scans(
        ...
        println!("Comparing scans: '{current_id}' vs '{previous_id}'");
    ...
    // The shells hold the live scan as a shared `Arc`; taking it by value keeps
    // every call site clone-free and is part of the shipped signature.
    ...
    pub fn scan_task_diff(
        ...
        let current_result = current.results.get(task_id).ok_or_else(|| {
            DiagError::storage(
                "scan_task_diff",
                format!("Task '{task_id}' is not present in the current scan"),
            )
            .to_string()
        })?;
        let previous_result = previous.results.get(task_id).ok_or_else(|| {
            DiagError::storage(
                "scan_task_diff",
                format!("Task '{task_id}' is not present in the previous scan"),
            )
            .to_string()
        })?;
        Ok(TaskDiffDetail {
    ...
    #[must_use]
    pub fn compute_comparison(current: ScanRecord, previous: ScanRecord) -> ComparisonResult {
    ...
    pub fn compute_comparison_with_catalog(
        ...
        for task_id in all_task_ids {
            ...
            let task_name = task_info.map_or_else(|| task_id.clone(), |t| t.name.clone());
            let category = task_info.map_or_else(|| "Unknown".to_string(), |t| t.category.clone());
            ...
            match (current_result, previous_result) {
                (Some(current), Some(previous)) => {
                    ...
                    } else if current.success && !previous.success {
    ...
    fn json_numbers_equal(a: &serde_json::Number, b: &serde_json::Number) -> bool {
        ...
        match (a.as_f64(), b.as_f64()) {
            // Exact equality is the point: this decides whether two stored JSON
            // numbers are the SAME value, not whether they are close enough.
    ...
    pub fn clear_history(&self) -> Result<(), String> {
        ...
        println!("Clear history completed: {deleted_count} deleted, {failed_count} failed");

        if failed_count > 0 {
            Err(DiagError::storage(
                "clear_history",
                format!("Cleared {deleted_count} scans, but {failed_count} failed to delete"),
            )
            .into())
        } else {
            Ok(())
