//! The diagnostic task catalog and the task-id dispatch table.
//!
//! `run_diagnostic_task` surfaces collector errors inside the returned
//! `TaskResult`; the one fallible helper here forwards its message verbatim, so
//! a `# Errors` section would only restate it.
...
/// Builds a successful/failed [`TaskResult`] without touching the system.
///
/// Test-support constructor kept `pub` (rather than `#[cfg(test)]`) because it
/// is used from the Tauri shell's own test modules, which live in a different
/// crate and therefore never see this crate's `cfg(test)`.
#[must_use]
...
// One flat, auditable table: every shipped task id, name, category and admin
...
// One exhaustive task-id dispatch table plus its command fallbacks; splitting it
// would only hide which ids are handled. `as_millis()` never exceeds u64 for a
// single task run.
...
fn decode_command_output(bytes: &[u8]) -> String {
    wfdiag_native_core::security::decode_windows_output(bytes)
}
