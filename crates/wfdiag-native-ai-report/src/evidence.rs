...
pub struct EvidenceRequest<'a, R = TaskResult> {
    pub question: &'a str,
...
impl EvidencePolicy {
    #[must_use]
    pub fn compact(max_chars: usize) -> Self {
        Self {
            max_chars,
...
// `EvidenceRequest` is a bundle of borrows built inline at every call site;
// taking it by value is the shipped signature both shells construct against.
#[allow(clippy::needless_pass_by_value)]
pub fn build_compact_evidence<R>(
...
// Candidate construction is one deterministic ordering pass; splitting it
// would spread the packet's selection rules across several functions.
    ...
    R: Borrow<TaskResult>,
{
    ...
    for issue in request.issues {
        ...
        let priority = if explicitly_preferred {
            ...
            // Kept as an explicit priority table: arms that share a number
            // today are separate rules and are tuned independently.
...
fn render_record(record: &EvidenceRecord) -> String {
    ...
    let classification = record.severity.as_deref().map_or_else(
        || format!("{}/{}", kind_name(record.kind), status_name(record.status)),
        |severity| {
            format!(
                "{}/{}/{}",
...
fn render_field_fragments(included: &[String], omitted: usize) -> String {
    ...
    if omitted > 0 {
        ...
        let _ = write!(rendered, "(+{omitted} fields)");
    }
    rendered
}
...
fn query_terms(text: &str) -> BTreeSet<String> {
        ...
        .filter(|token| token.len() >= 3 && !is_stop_word(token))
    {
        ...
        if let Some(synonyms) = synonyms_for(&token) {
            terms.extend(synonyms.iter().map(std::string::ToString::to_string));
        }
    }
    terms
}
